#include "gyro.h"
#include "rx.h"
#include "sd_card.h"

constexpr uint32_t LOOP_PERIOD_US = 1000; // 1 kHz

void stopProgram()
{
  if (Serial.available())
  {
    char c = Serial.read();

    if (c == 'q')
    {
      logFileIMU.flush();
      logFileIMU.close();

      logFileRX.flush();
      logFileRX.close();

      while(1);
    }
  }
}

void setup()
{
  Serial.begin(115200);

  pinMode(CS_IMU, OUTPUT);
  digitalWrite(CS_IMU, HIGH);

  SPI.begin();
  delay(50);

  // Gyro setup
  setupGyro();

  // SD card setup
  setupSD();

  // Gyro setup
  setupRx();

  float x = 54.2;
}

void loop()
{
  // uint32_t t = micros();

  // rx
  if(readSBUS())
  {
    decodeSBUS(frame);
    printRx(0);

    // uint32_t t = micros();
    // loopSD_RX(rx);
    // uint32_t dt = micros() - t;
    // Serial.print("dt_Gy ");
    // Serial.println(dt);

    // static uint32_t t_rx = micros();
    // uint32_t dt_rx = micros() - t_rx;
    // t_rx = micros();
    // Serial.print("dt_rx ");
    // Serial.println(dt_rx);

    // uint32_t t_rx = micros();
    // uint32_t dt_rx = t_rx - t;
    // Serial.print("dt_rx ");
    // Serial.println(dt_rx);
  }

  // Gyro
  static uint32_t nextLoop = micros() + LOOP_PERIOD_US;
  if ((int32_t)(micros() - nextLoop) < 0)
    return;

  nextLoop += LOOP_PERIOD_US;

  Gyro raw = readGyro();
  Gyro filteredGyro = filterGyro(raw);

  uint32_t t = micros();
  loopSD_IMU(filteredGyro);;
  uint32_t dt = micros() - t;
  Serial.print("dt_IMU ");
  Serial.println(dt);

  printGyro(0, filteredGyro);

  // static uint32_t t_gy = micros();
  // uint32_t dt_gy = micros() - t_gy;
  // t_gy = micros();
  // Serial.print("dt_gy ");
  // Serial.println(dt_gy);

  // type q in the serial monitor to stop the program
  // and flush the data in the sd card
  stopProgram();
}






